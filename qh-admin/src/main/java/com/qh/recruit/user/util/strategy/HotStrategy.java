 ...
 */
@Component
public class HotStrategy implements RecommendStrategy{
    private final RecommendServiceImpl recommendService;

    public HotStrategy(RecommendServiceImpl recommendService) {
    ...
    public AjaxResult recommend(LoginUser loginUser,List<UserJob> userJobs) {
        ...
        List<JobScore> jobScores = userJobs.stream().map(job -> {
                    ...
                    baseScore += recommendService.calculateHotMatchScore(loginUser, job) * 2.0;
