 ...
 */
@Component
public class WelfareStrategy implements RecommendStrategy{

    private final RecommendServiceImpl recommendService;
    @Autowired
    public WelfareStrategy(RecommendServiceImpl recommendService) {
    ...
    public AjaxResult recommend(LoginUser loginUser, List<UserJobDto> userJobs) {
        ...
        List<JobScore> jobScores = userJobs.stream().map(job -> {
                    ...
                    // 根据优先条件计算权重
                    // 技能匹配度权重加倍
                    baseScore += recommendService.calculateSkillMatchScore(loginUser, job) ;
                    baseScore += recommendService.calculateHotMatchScore(loginUser, job) ;
                    baseScore += recommendService.calculateWelfareMatchScore(loginUser, job,size) * 2.0;
